...
if exist "C:\Temp\usb-plugged-out-lock%1%2.txt" goto alreadyrunning

set /a randomnum=%RANDOM%
echo %randomnum%>C:\Temp\usb-plugged-out-lock%1%2.txt
title usb-plugged-in-%randomnum%.bat
timeout 2
set /p testisme= <C:\Temp\usb-plugged-out-lock%1%2.txt
echo %testisme%
echo %randomnum%
if not %testisme%==%randomnum% goto alreadyrunning

del C:\Temp\usb-plugged-out-lock%1%2.txt
...
IF %1=="2833" (
  IF %2=="0330" (
    ...
    Start "" "C:\Program Files\NiceHash Miner 2\NiceHash Miner 2.exe"
    timeout 5
    del C:\Temp\usb-plugged-out-lock%1%2.txt
    exit
  )
